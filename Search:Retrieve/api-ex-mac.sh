...
pages=$(curl 'http://eco.canadiana.ca/search?q=aboriginal&field=&so=score&df=1914&dt=1918&fmt=json' | jq '.pages')
...
do
        curl 'http://eco.canadiana.ca/search/'${i}'?q=aboriginal&field=&so=score&df=1914&dt=1918&fmt=json' | jq '.docs[] | {key}' >> results.txt
done
